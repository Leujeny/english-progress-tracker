import TypeAvatar from "../avatar/typeAvatar";

interface Props {
    datas: any,
}

export default function ViewedRessourceCard({ datas }: Props) {
    return (
        <div className="card bg-base-100 shadow-sm col-span-4 lg:col-span-2">
            <div className="card-body">
                <h2 className="card-title">My recent viewed ressources</h2>
                <ul className="list rounded-box">
                    {datas && datas.map((data: any) => (
                        <li className="list-row" key={data.id}>
                            <TypeAvatar type={data.type} />
                            <div className="my-auto">
                                <p>{data.name}</p>
                                <p className="text-xs uppercase font-semibold opacity-60">{data.date}</p>
                            </div>
                        </li>
                    ))}
                </ul>
            </div>
        </div>
    );
}
